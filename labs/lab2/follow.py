...
class JacobianDemo():
    ...
    def ellipse(t,f=0.5,ry=.15,rz=.10):
        ...
        # TODO: replace these!
        xdes = x0 + np.array([rz * cos(f * t), ry * sin(f * t), 0])
        
        # Velocity is the time derivative of the position
        vdes = np.array([-rz * f * sin(f * t), ry * f * cos(f * t), 0])
    ...
    def line(t,f=1.0,L=.15):
        ...
        # Define angular frequency (omega) since f is in Hz
        omega = (1/9) * np.pi * f

        # F = 2pi / omega
        
        # Create sinusoidal motion along the y-axis with amplitude L/2
        # Position is the starting point plus the line offset
        xdes = x0 + np.array([0, (L/2) * sin(omega * t), 0])
        
        # Velocity is the time derivative of the position
        vdes = np.array([0, (L/2) * omega * cos(omega * t), 0])
